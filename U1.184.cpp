...
struct person
{
    ...
    string toString()
    {
        return name + " " + to_string(gebtag) + "." + to_string(gebmon) + "." + to_string(gebjahr);
    }
};
...
int print_list(string message = "User list")
{
    cout << "======" + message + "======\n";
    current = root;
    cout << setw(32) << current->toString() << setw(32) << "(" << current->prev << "->" << current << "->" << current->next << ")" << endl;
    ...
    if (serach_name.length() > 0)
    {
        ...
        if (current != NULL)
        {
            ...
            if (answer == "y")
            {
                ...
                if (current == NULL)
                {
                    // This is the first element and we only need to reset the root pointer
                ...
                {
                    // This is not the first element and we need to reset the next pointer of the previous element
